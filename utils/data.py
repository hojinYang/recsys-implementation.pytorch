...
class UserItemRatingDataset(Dataset):
  ...
  def __getitem__(self, index):
    print(type(self.target_tensor[index]))
    return Variable(torch.LongTensor(self.user_tensor[index])),Variable(torch.LongTensor(self.item_tensor[index])), Variable(torch.FloatTensor(self.target_tensor[index]))
...
class UserItemDataset(Dataset):
  ...
  def __getitem__(self, index):
    print(type(self.user_tensor[index]))
    return Variable(torch.LongTensor(self.user_tensor[index])), Variable(torch.LongTensor(self.item_tensor[index]))
  ...
  def instance_test_loader(self, num_negatives, batch_size):
    ...
    if num_negatives == -1:
      for row in self.test_interacted_status.itertuples():
        ni = list(map(int, row.negative_samples))
